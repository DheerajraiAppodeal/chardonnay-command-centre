...
function formatDay(d) { return `${d.getDate()} ${MONTH_NAMES[d.getMonth()]}`; }
...
for (let w = 0; w < 10; w++) {
  ...
  WEEK_GROUPS.push({
    weekIdx: w,
    label: `${MONTH_NAMES[days[0].getMonth()]} W${Math.ceil(days[0].getDate() / 7)}`,
    days,
  });
}
...
// ── Status ────────────────────────────────────────────────────────────────
const S = {
  "not-started": { label: "Not started", bar: "#CBD0DC", text: "#8B93A8" },
...
// ── OOO helpers ───────────────────────────────────────────────────────────
function getDayOOOInfo(absIdx) {
  if (absIdx < 0 || absIdx >= ALL_DAYS.length) return { people: [], holidays: [] };
  const dateStr = isoDate(ALL_DAYS[absIdx]);
  return {
    people:   getOOOPeopleOnDate(dateStr),
    holidays: getPublicHolidayOnDate(dateStr),
  };
}
...
function Pill({ label, color, bg, small }) {
  return (
    <span style={{
      ...
      background: bg || color + "20", color, whiteSpace: "nowrap",
    }}>
...
// ── OOO Tooltip ───────────────────────────────────────────────────────────
function OOOTooltip({ people, holidays, T }) {
  if (!people.length && !holidays.length) return null;
  return (
    <div style={{
      position: "absolute", bottom: "100%", left: "50%", transform: "translateX(-50%)",
      background: T.isDark ? "#1E2A3A" : "#1E2530",
      color: "#E8ECF2", borderRadius: 6, padding: "5px 8px",
      fontSize: 9, whiteSpace: "nowrap", zIndex: 100,
      boxShadow: "0 4px 12px rgba(0,0,0,0.3)",
      pointerEvents: "none",
    }}>
      {holidays.map((h, i) => <div key={i} style={{ color: "#4A9A9A", fontWeight: 700 }}>🏖 {h.name}</div>)}
      {people.length > 0 && <div style={{ color: "#F4A428", marginTop: holidays.length ? 3 : 0 }}>OOO: {people.join(", ")}</div>}
    </div>
  );
}

// ── Day Column (with OOO overlay) ─────────────────────────────────────────
function DayColumn({ absIdx, relIdx, day, T, height }) {
  const [hover, setHover] = useState(false);
  const { people, holidays } = getDayOOOInfo(absIdx);
  const isToday   = absIdx === TODAY_IDX;
  const isMon     = relIdx % 5 === 0;
  const hasOOO    = people.length > 0;
  ...
  return (
    <div
      ...
      style={{
        position: "absolute", left: relIdx * DAY_W, top: 0, height,
        width: DAY_W,
        borderLeft: isMon ? `1.5px solid ${T.border}` : `1px solid ${T.isDark ? "#1A2535" : "#ECEEF2"}`,
        background: isHoliday
          ? "rgba(74,154,154,0.08)"
          : isToday
            ? "rgba(244,164,40,0.06)"
            : hasOOO
              ? "rgba(192,80,64,0.05)"
              : "transparent",
        cursor: (hasOOO || isHoliday) ? "default" : "default",
        zIndex: hover && (hasOOO || isHoliday) ? 50 : 0,
      }}
    >
      {/* Holiday stripe */}
      {isHoliday && (
      ...
      {hasOOO && !isHoliday && (
        <div style={{
          ...
          {people.slice(0, 3).map((_, i) => (
            <div key={i} style={{ width: 3, height: 3, borderRadius: "50%", background: "#C05040", opacity: 0.7 }} />
          ))}
        </div>
      )}
      {/* Today line */}
      {isToday && (
        <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: 2, background: "#F4A428", opacity: 0.7 }} />
      )}
      {/* Tooltip on hover */}
      {hover && (hasOOO || isHoliday) && (
        <div style={{ position: "relative" }}>
          <OOOTooltip people={people} holidays={holidays} T={T} />
        </div>
      )}
    </div>
  );
...
function VersionHeader({ label, color, features, T, windowStart, expanded, onToggle }) {
  return (
    <div onClick={onToggle} style={{
      display: "flex", alignItems: "center", height: GROUP_H,
      cursor: "pointer",
...
// ── Feature Row ───────────────────────────────────────────────────────────
function FeatureRow({ feature, gameColor, T, windowStart, visibleDays, isSelected, onSelect }) {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${T.isDark ? "#141C28" : "#F0F2F5"}` }}>
      ...
      <div onClick={() => onSelect(isSelected ? null : feature.id)} style={{
        display: "flex", alignItems: "center", height: FEAT_H,
        cursor: "pointer",
        ...
        <div style={{ flex: 1, position: "relative", height: "100%" }}>
          {(() => {
            ...
            const left = (Math.min(...all) - windowStart) * DAY_W;
            const width = (Math.max(...all) - Math.min(...all)) * DAY_W;
            if (width <= 0) return null;
            return (
              <div style={{
                position: "absolute", left: Math.max(0, left) + 2,
                width: Math.max(4, width - 4),
                top: "50%", transform: "translateY(-50%)",
                height: 3, borderRadius: 99, background: gameColor + "40",
                pointerEvents: "none",
              }} />
            );
          })()}
      ...
      {TRACKS.map(track => {
        ...
        const cl = Math.max(0, leftPx);
        const cr = Math.min(WINDOW_DAYS * DAY_W, leftPx + widthPx);
        const cw = cr - cl;
        const visible = active && cw > 0;

        // Check if any person on this track has OOO during this feature's days
        const trackOOODays = active ? visibleDays.filter((_, i) => {
          const absIdx = windowStart + i;
          if (absIdx < dayStart || absIdx >= dayEnd) return false;
          const { people: oooPeople } = getDayOOOInfo(absIdx);
          return people.some(p => oooPeople.includes(p));
        }).length : 0;
        ...
        return (
          <div key={track} style={{ display: "flex", alignItems: "center", height: ROW_H, marginBottom: TRACK_GAP }}>
            <div style={{ width: LABEL_W, flexShrink: 0, paddingLeft: 28, display: "flex", alignItems: "center", gap: 5 }}>
              ...
              {trackOOODays > 0 && active && (
                <span title={`${trackOOODays} OOO day${trackOOODays > 1 ? "s" : ""} overlap`}
                  style={{ fontSize: 8, color: "#C05040", fontWeight: 700 }}>⚠{trackOOODays}d</span>
              )}
            </div>
            <div style={{ flex: 1, position: "relative", height: "100%" }}>
              {visible && (
                <div style={{
                  ...
                  background: style.bar, borderRadius: 4, opacity: 0.88,
                  display: "flex", alignItems: "center", paddingLeft: 6,
                  overflow: "hidden", boxShadow: `0 1px 3px ${style.bar}44`,
                }}>
                  {cw > 40 && people.length > 0 && (
              ...
              {td.status === "not-started" && (
                <div style={{ position: "absolute", left: 4, top: "50%", transform: "translateY(-50%)", fontSize: 8, color: T.faint, fontStyle: "italic", opacity: 0.5 }}>—</div>
              )}
            </div>
          </div>
        );
...
function DetailPanel({ feature, gameColor, T, onClose }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, padding: "16px 14px", alignSelf: "start",
      ...
      {TRACKS.map(track => {
        const td = feature.tracks[track] || { status: "not-started" };
        const tm = TRACK_META[track];
        const s  = S[td.status] || S["not-started"];
        const people = td.people || [];
...
function ResourceView({ features, T, windowStart, visibleDays }) {
  ...
  // Add OOO-only people (those with no active features but have OOO)
  Object.keys(TEAM_OOO).forEach(person => {
    if (!personMap[person]) personMap[person] = [];
  });
          ...
          <div key={person} style={{ borderBottom: `1px solid ${T.border}` }}>
            ...
            <div style={{ display: "flex", alignItems: "center", height: FEAT_H }}>
              <div style={{ width: LABEL_W, flexShrink: 0, paddingLeft: 12, display: "flex", alignItems: "center", gap: 7 }}>
                <div style={{ width: 22, height: 22, borderRadius: "50%", background: loadColor + "25", border: `1.5px solid ${loadColor}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontSize: 9, fontWeight: 700, color: loadColor }}>{person.charAt(0)}</span>
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, color: T.text }}>{person}</span>
                {load > 0 && (
                  <span style={{ fontSize: 9, fontWeight: 700, color: loadColor, background: loadColor + "20", padding: "1px 5px", borderRadius: 99 }}>
                    {load} active
                  </span>
                )}
              </div>
              {/* OOO markers for this person */}
              <div style={{ flex: 1, position: "relative", height: FEAT_H }}>
                {visibleDays.map((day, i) => {
                  const absIdx = windowStart + i;
                  const { people: oooPeople, holidays } = getDayOOOInfo(absIdx);
                  const isPersonOOO = oooPeople.includes(person);
                  const isHol = holidays.length > 0;
                  if (!isPersonOOO) return null;
                  return (
                    <div key={i} style={{
                      position: "absolute",
                      left: i * DAY_W + 2, width: DAY_W - 4,
                      top: 4, height: FEAT_H - 8,
                      background: isHol ? "#4A9A9A30" : "#C0504025",
                      border: `1px solid ${isHol ? "#4A9A9A" : "#C05040"}44`,
                      borderRadius: 3,
                      display: "flex", alignItems: "center", justifyContent: "center",
                    }}>
                      <span style={{ fontSize: 8, color: isHol ? "#4A9A9A" : "#C05040" }}>
                        {isHol ? "🏖" : "OOO"}
                      </span>
                    </div>
                  );
            ...
            {/* Assignment bars */}
            {assignments.map((a, i) => {
              const s = S[a.td.status] || S["not-started"];
              const tm = TRACK_META[a.track];
              const leftPx  = (a.td.weekStart * 5 - windowStart) * DAY_W;
              const widthPx = Math.max((a.td.weekEnd - a.td.weekStart) * 5 * DAY_W, DAY_W);
                ...
                <div key={i} style={{ display: "flex", alignItems: "center", height: ROW_H, marginBottom: TRACK_GAP }}>
                  ...
                  <div style={{ flex: 1, position: "relative", height: "100%" }}>
                    <div style={{
                      position: "absolute", left: cl + 2, width: cw - 4,
                      top: 3, height: ROW_H - 6,
                      background: s.bar, borderRadius: 4, opacity: 0.85,
...
// ── OOO Summary Bar ───────────────────────────────────────────────────────
function OOOSummaryBar({ windowStart, visibleDays, T }) {
  const oooWeeks = {};
  visibleDays.forEach((day, i) => {
    const absIdx = windowStart + i;
    const weekIdx = Math.floor(absIdx / 5);
    const { people, holidays } = getDayOOOInfo(absIdx);
    if (!oooWeeks[weekIdx]) oooWeeks[weekIdx] = { people: new Set(), holidays: [], days: 0 };
    people.forEach(p => oooWeeks[weekIdx].people.add(p));
    holidays.forEach(h => { if (!oooWeeks[weekIdx].holidays.includes(h.name)) oooWeeks[weekIdx].holidays.push(h.name); });
    if (people.length || holidays.length) oooWeeks[weekIdx].days++;
  });

  const hasAnyOOO = Object.values(oooWeeks).some(w => w.people.size > 0 || w.holidays.length > 0);
  if (!hasAnyOOO) return null;

  return (
    <div style={{
      background: T.isDark ? "#0C1420" : "#F0F6FF",
      border: `1px solid ${T.border}`, borderRadius: 10,
      padding: "10px 14px", display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center",
    }}>
      <span style={{ fontSize: 9, fontWeight: 700, color: T.faint, textTransform: "uppercase", letterSpacing: "0.1em", flexShrink: 0 }}>
        🗓 This window
      </span>
      {Object.entries(oooWeeks).map(([wk, data]) => {
        if (!data.people.size && !data.holidays.length) return null;
        const wg = WEEK_GROUPS[parseInt(wk)];
        return (
          <div key={wk} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 9 }}>
            <span style={{ fontWeight: 700, color: T.muted }}>{wg?.label}:</span>
            {data.holidays.map((h, i) => (
              <span key={i} style={{ color: "#4A9A9A", fontWeight: 700 }}>🏖 {h}</span>
            ))}
            {data.people.size > 0 && (
              <span style={{ color: "#C05040" }}>
                OOO — {[...data.people].join(", ")}
              </span>
            )}
          </div>
        );
...
export default function RoadmapPage({ T }) {
  ...
  // Group visible days by week
  const visibleWeeks = [];
  visibleDays.forEach((day, i) => {
    const absIdx  = windowStart + i;
    const weekIdx = Math.floor(absIdx / 5);
    if (!visibleWeeks.length || visibleWeeks[visibleWeeks.length - 1].weekIdx !== weekIdx) {
  ...
  // Calculate total content height for DayColumn overlay
  const contentHeight = 2000; // generous, columns are absolute
    ...
    <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button onClick={() => setShowOOOPanel(v => !v)} style={{
            padding: "5px 14px", fontSize: 11, fontWeight: 700, borderRadius: 99, cursor: "pointer",
            border: `1.5px solid ${showOOOPanel?T.accent:T.border}`,
            background: showOOOPanel?T.navActiveBg:T.surface,
            color: showOOOPanel?T.accent:T.muted,
          }}>🏖 OOO / Holidays</button>
        ...
        {/* OOO Summary */}
        {showOOOPanel && <OOOSummaryBar windowStart={windowStart} visibleDays={visibleDays} T={T} />}
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
          {Object.entries(S).map(([status, st]) => (
            <div key={status} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: T.muted }}>
              <div style={{ width: 10, height: 6, borderRadius: 2, background: st.bar }} />{st.label}
            </div>
          ))}
          <div style={{ width: 1, height: 12, background: T.border }} />
          <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#4A9A9A" }}>
            <div style={{ width: 10, height: 6, borderRadius: 2, background: "#4A9A9A" }} />Public holiday
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#C05040" }}>
            <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#C05040", opacity: 0.7 }} />OOO
          </div>
        </div>
        ...
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", borderBottom: `2px solid ${T.border}`, background: T.isDark?"#0C1220":"#F4F6F9" }}>
            ...
            <div style={{ flex: 1, display: "flex" }}>
              {visibleDays.map((day, i) => {
                ...
                const isToday = absIdx === TODAY_IDX;
                const isMon   = i % 5 === 0;
                const { holidays } = getDayOOOInfo(absIdx);
                const isHol   = holidays.length > 0;
                return (
                  <div key={i} style={{
                    width: DAY_W, flexShrink: 0, textAlign: "center", padding: "3px 2px",
                    borderLeft: isMon?`1.5px solid ${T.border}`:`1px solid ${T.isDark?"#1A2535":"#ECEEF2"}`,
                    background: isHol?"rgba(74,154,154,0.1)":isToday?"rgba(244,164,40,0.12)":"transparent",
                  }}>
                    <div style={{ fontSize: 8, fontWeight: isToday?700:400, color: isHol?"#4A9A9A":isToday?T.accent:T.faint }}>
                      {DAY_NAMES[i%5]}
                    </div>
                    <div style={{ fontSize: 9, fontWeight: isToday||isHol?700:600, color: isHol?"#4A9A9A":isToday?T.accent:T.text }}>
                      {day.getDate()}
                    </div>
                    {isHol && <div style={{ fontSize: 7, color: "#4A9A9A", lineHeight: 1 }}>{holidays[0].name.substring(0,6)}</div>}
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ position: "relative" }}>
            {/* Day column overlays */}
            <div style={{ position: "absolute", left: LABEL_W, top: 0, bottom: 0, right: 0, pointerEvents: "none" }}>
              {visibleDays.map((day, i) => (
                <DayColumn key={i} absIdx={windowStart+i} relIdx={i} day={day} T={T} height={contentHeight} />
              ))}
            </div>
            ...
            {viewMode === "features" ? (
              Object.entries(filteredGroups).map(([version, feats]) => {
                ...
                const isCol  = collapsed[version];
                return (
                  <div key={version}>
                    <VersionHeader label={version} color={vColor} features={feats} T={T}
                      windowStart={windowStart} expanded={!isCol}
                      onToggle={() => setCollapsed(c => ({...c,[version]:!c[version]}))} />
                    {!isCol && feats.map(f => (
                      <FeatureRow key={f.id} feature={f} gameColor={vColor} T={T}
                        windowStart={windowStart} visibleDays={visibleDays}
                        isSelected={selected===f.id} onSelect={setSelected} />
                    ))}
                  </div>
                );
              })
              ...
              <ResourceView
                ...
                T={T} windowStart={windowStart} visibleDays={visibleDays}
              />
            )}
          </div>
        </div>
